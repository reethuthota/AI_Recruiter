...
conn = MongoClient('mongodb+srv://ree:ree123@ai-recruiter.robdv1v.mongodb.net/AI-Recruiter')
...
def view_applications(job_id):
    job = jobs.find_one({'_id': ObjectId(job_id)})
    applications = db[job['job_collection_name']].find().sort('total_score', -1)
    
    if not applications:
        flash("No applications found for this job.", "error")
    ...
    if request.method == 'POST':
        if 'send_emails_button' in request.form:
            if job.compiled != 'yes':
                flash("The job has not been compiled yet. Emails cannot be sent out.", "error")
                return redirect(url_for('view_applications', job_id=job_id))
